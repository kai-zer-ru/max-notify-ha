...
- **Одно сообщение с несколькими вложениями** — там, где это поддерживает интерфейс действий (список файлов вместо одного файла).
...
| **send_document** | Файл-документ; те же идеи, что у фото. |
| **send_video** | Видео (форматы вроде mp4, mov, webm, mkv). |
| **delete_message** | Удалить сообщение по `message_id` (ограничения по времени задаёт платформа / сервис). |
| **delete_last_outgoing_message** | Автоматически найти и удалить последнее исходящее сообщение бота: **только для групповых чатов** (Official Max API). Для личных чатов API не даёт читать историю по `recipient_id`. |
| **edit_message** | Изменить текст (**в т.ч. формат** в расширенных), кнопки сообщения или снять их (**`remove_buttons`**). |

В расширенных параметрах у **медиа**-действий дополнительно при необходимости задаются **авторизация для скачивания по ссылке**, **отключение проверки SSL** (только если понимаете риск), число **повторов** отправки, пока вложение «готовится» на сервере.
