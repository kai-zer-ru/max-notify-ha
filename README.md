...
- [Сенсоры идентификаторов сообщений](#message-id-sensors)
...
- **Сенсоры:** для каждого добавленного чата — отдельные диагностические сенсоры **последнего входящего** и **последнего исходящего** `message_id` (удобно для `edit_message` / `delete_message`); запись ведётся **по паре «запись интеграции + recipient_id»** и только для **настроенных** получателей.
...
5. Укажите **`recipient_id`**, который выдал бот вместе с токеном: **положительный** — личный диалог (`user_id`), **отрицательный** — групповой чат (`chat_id`), если прокси выдал связку для группы. У **`notify.a161.ru`** это **одна пара «ключ + получатель»**: сменить чат или пользователя **в той же записи** нельзя — нужен **новый ключ** у бота и **новая** запись MaxNotify.

**Шестерёнка / настройки** записи: формат сообщений, режим приёма, при Polling — интервал опроса, период неактивности и клавиатура. Токен можно сменить через **Перенастроить** (⋮). Чтобы сменить получателя (`user_id` / группу), удалите запись и создайте заново с новой выдачей у бота.
...
В режиме **`notify.a161.ru`** в **одной** записи интеграции бывает **только один** получатель — тот, что выдан вместе с токеном (личный **`user_id` > 0`** или группа **`< 0`**, если прокси это поддерживает для вашей выдачи). Отправить «в другой чат» той же записью нельзя: нужен **другой API-ключ** и **отдельная** интеграция.
...
<a id="message-id-sensors"></a>

## 🆔 Сенсоры идентификаторов сообщений

Для **каждого** чата/получателя, добавленного в запись интеграции (subentry), создаются сенсоры **последнего входящего** и **последнего исходящего** идентификатора сообщения Max. Их можно использовать в сценариях правки и удаления сообщений.

**Как обновляется состояние**

- Значения хранятся **отдельно по записи интеграции и по `recipient_id`** (положительный User ID для лички, отрицательный Chat ID для группы), а не одним общим значением на всю запись при нескольких чатах.
- Сенсор обновляется **только если** `recipient_id` сообщения **есть среди настроенных** в этой записи чатов. Входящее из «чужого» чата или попытка зафиксировать исходящее для ID, который не добавлен в интеграцию, **не меняют** ни одного такого сенсора.
- Для совместимости остаются устаревшие сенсоры «на всю запись» без привязки к чату: они дублируют значение **только если** в записи настроен **ровно один** получатель (чтобы не подставлять ID из «не того» чата).

**Другой настроенный чат при той же записи (только официальный API)**

Для записи **официального API** с **несколькими** добавленными чатами в сервисах `max_notify.*` можно выбрать цель **`entity_id`** одной сущности `notify`, а в дополнительных параметрах указать **`recipient_id` другого уже настроенного** чата — сообщение уйдёт туда, а исходящий ID отобразится в сенсорах этого чата. Поле **`recipient_id` имеет смысл указывать только когда оно не совпадает** с получателем выбранной сущности `notify` (как в подсказках к полю в UI). Если несколько записей MaxNotify и не задан `config_entry_id`, запись для отправки **определяется по выбранной** сущности `notify`.

Для **`notify.a161.ru`** в записи **один** получатель на ключ; **`recipient_id` / `chat_id` / `user_id`** в сервисах должны совпадать с ним (переключение на «другой чат» — **другая** интеграция с **другим** токеном).

**Стандартный `notify.send_message`**

Встроенный сервис Home Assistant для сущности `notify` передаёт в интеграцию только текст и заголовок — **переопределить получателя через него нельзя**. Для **официального API** с несколькими чатами, чтобы уйти в другой настроенный чат и обновить нужные сенсоры, используйте **`max_notify.send_message`** (или другой сервис домена `max_notify`) с `entity_id` и при необходимости `recipient_id`. Для **`notify.a161.ru`** цель задаётся одной сущностью `notify` этой записи; другой чат — отдельная запись интеграции.
...
**В сервисе** `max_notify.send_message` можно передать параметр **`buttons`** — сообщение уйдёт с указанной клавиатурой. Формат: список рядов, каждый ряд — список кнопок `{type: "callback"|"message"|"link", text: "Подпись", payload?: "..."}` или для link `{type: "link", text: "...", url: "https://..."}`. Цель задаётся **`entity_id`** сущностей `notify` и/или **`config_entry_id`** с **`recipient_id`** (или устаревшие `chat_id` / `user_id` до версии 1.5.0).
...
| `title` | Заголовок |
| `entity_id` | Одна или несколько сущностей `notify` MaxNotify |
| `config_entry_id` | ID записи интеграции — если несколько записей MaxNotify, нет цели `entity_id` или так удобнее в сценарии; задаётся в «Дополнительно», для official часто вместе с `recipient_id` |
| `recipient_id` | Универсальный ID: **> 0** — личка (User ID), **< 0** — группа (Chat ID). **Официальный API:** укажите **только если** цель **не** совпадает с получателем выбранной сущности `notify`. **`notify.a161.ru`:** у записи **один** получатель на токен — поле **не** используют для «переключения» на другой чат (нужна другая интеграция); при необходимости должен **совпадать** с настроенным id |
...
Файл по пути или URL отправляется как документ. Параметры: `file`, поля URL-авторизации как у `send_photo` (`url_auth_type`, `url_auth_login`, `url_auth_password`, `url_auth_token`, `url_basic_auth` — устарело), `caption`, цель как у `send_message` (`entity_id`, `config_entry_id`, `recipient_id` или устаревшие `chat_id` / `user_id` до 1.5.0), **`count_requests`** — см. описание у `send_photo`.
...
| `entity_id` | Сущности `notify` (как у `send_message`) |
| `recipient_id` | **Официальный API:** если чат операции **отличается** от получателя выбранной сущности `notify`. **`notify.a161.ru`:** обычно не нужен (один получатель на запись) |
| `config_entry_id` | Запись интеграции (если несколько и не задан однозначный `entity_id`) |
