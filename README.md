...
**Поддерживается Home Assistant** не ниже **2026.2.0** (более старые выпуски могут работать некорректно).
...
- [Где взять токен и ID](#где-взять-токен-и-id)
...
- Приём входящих — **WebSocket** (по умолчанию, если сервер разрешил) или **Long Polling**. Короткий опрос раз в несколько секунд больше не используется. WebHook в этом режиме нет.
- Перед отправкой файлов интеграция проверяет размер **до 10 МБ** на файл.
- Если выбран **Long Polling** и долго не было ни входящих сообщений, ни исходящих **с кнопками**, интеграция может **сама переключить приём на «только отправка»** (чтобы снова слушать чат, включите приём в настройках записи). Для **WebSocket** такого автоотключения нет.
...
- **Медиа по ссылке:** если в действии указан `http`/`https`, файл **один раз** скачивается во временную папку Home Assistant и уже с диска уходит в Max. При рассылке в несколько чатов повторного скачивания той же ссылки нет; после отправки временный файл удаляется. Путь к файлу на диске HA по-прежнему можно указать напрямую.
...
У **официального API** есть **Long Polling** и **WebHook**; у **notify.a161.ru** входящие идут по **WebSocket** или **Long Polling**. **Группы и каналы** (отрицательный ID чата) поддерживаются в обоих типах подключения в рамках возможностей платформы; остальные ограничения notify.a161.ru — в разделе [Сервис notify.a161.ru](#сервис-notifya161ru).
...
2. Введите токен, формат сообщений и режим: **только отправка**, **WebSocket** (по умолчанию) или **Long Polling**. Если токен неактивен, приём не подключается.
3. Для Long Polling интервал опроса берётся **с сервера** (его не выбирают); можно задать **limit** (сколько событий за раз). Срок автоотключения тоже задаёт сервер и действует только для Long Polling. Правило переключения на «только отправка» — в [блоке про сервис](#сервис-notifya161ru).
...
| **send_photo** | Фото: файл на диске Home Assistant или ссылка (ссылка сначала скачивается во временный файл); можно несколько файлов; **подпись**; в расширенных — **формат**, **`notify`**, клавиатура, **`buttons`**, повторы при обработке вложения на стороне Max. |
| **send_document** | Один файл-документ за вызов (только поле **`file`**, не **`files`**); путь или ссылка (ссылка — через временный файл); подпись, формат, **`notify`**, клавиатура и **`buttons`** — как у фото. |
| **send_video** | Видео (форматы вроде mp4, mov, webm, mkv); путь или ссылка (ссылка — через временный файл); в расширенных — **формат**, **`notify`**, клавиатура и **`buttons`**. |
...
- **User ID и Chat ID:** например бот [CHECK ID](https://max.ru/id222312277810_1_bot) в Max (по пересланному сообщению). Список чатов через `GET /chats` с июня 2026 **недоступен** — `chat_id` группы/канала берут из событий (`bot_added`, `message_created` и т.п.) при WebHook ([POST /subscriptions](https://dev.max.ru/docs-api/methods/POST/subscriptions)) или Long Polling ([GET /updates](https://dev.max.ru/docs-api/methods/GET/updates)), либо из поля `chat_id` / `recipient_id` в событии Home Assistant `max_notify_received`. Подробнее — [документация Max](https://dev.max.ru/docs-api).
