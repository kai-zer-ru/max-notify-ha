...
**Другой настроенный чат при той же записи**

В сервисах `max_notify.*` можно выбрать цель **`entity_id`** одной сущности `notify`, а в дополнительных параметрах указать **`recipient_id` другого уже настроенного** чата — сообщение уйдёт туда, а исходящий ID отобразится в сенсорах этого чата. Поле **`recipient_id` имеет смысл указывать только когда оно не совпадает** с получателем выбранной сущности `notify` (так же описано в подсказках к полю в UI). Если у вас несколько записей MaxNotify и не задан `config_entry_id`, запись для отправки **определяется по выбранной** сущности `notify`.

**Стандартный `notify.send_message`**

Встроенный сервис Home Assistant для сущности `notify` передаёт в интеграцию только текст и заголовок — **переопределить получателя через него нельзя**. Чтобы отправить в другой настроенный чат и обновить нужные сенсоры, используйте **`max_notify.send_message`** (или другой сервис домена `max_notify`) с `entity_id` и при необходимости `recipient_id`.
...
| `config_entry_id` | ID записи интеграции — если несколько записей MaxNotify, нет цели `entity_id` или так удобнее в сценарии; задаётся в «Дополнительно», обычно вместе с `recipient_id` |
| `recipient_id` | Универсальный ID получателя: **> 0** — User ID (личка), **< 0** — Chat ID (группа). Указывайте **только если** он **не совпадает** с получателем выбранной сущности `notify`; иначе поле можно не заполнять |
...
| `recipient_id` | Только если чат операции **отличается** от получателя выбранной сущности `notify` |
