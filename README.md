...
- Поддерживаются **текстовые сообщения**, **фото и файлы** (`send_photo`, `send_document`) и **видео** (`send_video`). На стороне интеграции проверяется размер: фото/файлы — до **4 МБ**, видео — до **10 МБ** (итоговые лимиты — у [notify.a161.ru](https://notify.a161.ru/)).
...
- На текущий момент **не работают**: `send_video`, `edit_message`, `delete_message`.
...
> Для режима `notify.a161.ru`: отправка текста, фото и файлов (`send_photo`, `send_document`); видео, редактирование и удаление сообщений — только у официального API `platform-api.max.ru`. Остальные возможности в списке ниже в основном относятся к `platform-api.max.ru`.
...
Доступны `max_notify.send_message`, **`send_photo`** и **`send_document`** (медиа — по правилам сервиса, см. выше). Редактирование и удаление сообщений, отправка видео — нет.
...
> Примечание: в режиме `notify.a161.ru` доступны также `send_photo` и `send_document`. Параметры клавиатуры/кнопок относятся к официальному API-домену `platform-api.max.ru`.
...
| `count_requests` | Для официального API: число попыток POST при ожидании обработки вложения (`attachment.not.ready`). Увеличьте для больших файлов (по умолчанию — 4). Для `notify.a161.ru` обычно достаточно одной попытки. |

### 📎 max_notify.send_document

Работает и для **официального API**, и для **`notify.a161.ru`** (с лимитом размера по правилам сервиса).

Файл по пути или URL отправляется как документ. Параметры: `file`, `caption`, `entity_id` (или config_entry_id + chat_id/user_id), **`count_requests`** — для официального API: число попыток при ожидании обработки вложения (для больших файлов).
