...
# [v2.3.0-beta3] — 2026-09-08

## Важно

- Если вы **перестали пользоваться** notify.a161.ru — всё равно обновите интеграцию. Сервису нужно понять, кто ещё на старых версиях.

## Добавлено

- В списке интеграций Home Assistant у MaxNotify теперь **своя иконка** (значок мессенджера Max), а не общая заглушка.
- В запросах к notify.a161.ru в заголовке **X-Client-Version** передаётся версия интеграции **без патча** (для 2.3.0-beta3 это `2.3`). Официальный API Max не менялся.
