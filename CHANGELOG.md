...
- Период автоотключения polling больше **не выбирается**. Срок берётся с сервера (`polling_inactivity_auto_disable_days`, например 7 суток) и показывается на отдельном информационном шаге. Если лимит на сервере подняли — он применяется после обновления возможностей, без перевыбора в настройках.
- В мастере и в настройках **notify.a161** убраны лишние тексты: на каждом экране только то, что к нему относится. Интервал опроса — на шаге интервала, автоотключение — на своём шаге, формат и режим — без чужих лимитов и без пояснений про WebSocket / смену ключа.

## Исправлено

- Раньше срок неактивности зажимался в **1–3 суток**, даже если сервер отдал 7. Теперь показывается и используется значение с сервера.
