...
- Те же заголовки (`X-RateLimit-Status`, `X-Retry-After-Seconds`) учитываются на опросе `/updates`, загрузке файлов и отправке: `REJECTED` / 429 — пауза из заголовка, `DELAYED` — не стучим чаще, чем сказал сервер.
...
- Входящие по WebSocket больше **не перезагружают** интеграцию. Раньше каждое сообщение записывалось в настройки записи, Home Assistant считал это сменой конфигурации и через полсекунды выгружал сокет.
