...
2. Скопируйте оба `.crt` в каталог additional_ca в конфигурации Home Assistant, например:
   - `/config/additional_ca/russian_trusted_root_ca_pem.crt`
   - `/config/additional_ca/russian_trusted_sub_ca_pem.crt`  
   (в File editor / Samba / SSH — эта папка должна лежать рядом с `configuration.yaml`.)
...
5. Придёт уведомление (в боковую панель) от интеграции **Additional CA** о том, что сертификаты не установлены и нужна перезагрузка. В этом уведомлении будут написаны имена сертификатов. Убедитесь, что это ваши сертификаты.
...
> **Какое у вас SSH-дополнение.** На HAOS чаще всего стоит **официальное** **Terminal & SSH** (hostname `core-ssh`, из магазина Home Assistant). У него **нет** переключателя «Режим защиты» и **нет** доступа к `docker` — это нормально. Прямой `curl` в таком терминале часто даёт SSL-ошибку; это **не** означает, что сертификаты не установились.
>
> Переключатель **«Режим защиты»** есть только у **community**-дополнения [**SSH & Web Terminal**](https://github.com/hassio-addons/addon-ssh) (другое дополнение, не путать с официальным). Если нужен `docker exec` из терминала и режим защиты **включён** — **отключите** его (не «переключайте» наугад: у кого-то он уже выключен, и случайное действие **включит** защиту обратно). После отключения **перезапустите** дополнение. Если режим защиты **уже выключен** — ничего не меняйте.
>
> **Порядок не меняется:** сначала сертификаты и [проверка SSL (п. 1)](#1-haos--docker--supervised--контейнер-homeassistant), **затем** обновление MaxNotify до **2.1.0+**. [П. 2](#2-в-home-assistant) (Перенастроить, тестовое сообщение) — уже **после** обновления. Ошибки `curl`/`docker` в официальном терминале сами по себе проверкой не являются.

### 1. HAOS / Docker / Supervised — контейнер `homeassistant`

Команду `docker exec` можно выполнить из community-дополнения **SSH & Web Terminal** (если **отключён** режим защиты — см. выше), из [отладочного SSH](https://developers.home-assistant.io/docs/operating-system/debugging/) (**22222** → `login`) или с любого хоста, где виден контейнер `homeassistant`. В **официальном** Terminal & SSH (`core-ssh`) `docker` недоступен — если нет другого способа запустить `docker exec`, дождитесь уведомления от **Additional CA** об установке сертификатов и переходите к обновлению MaxNotify; окончательную проверку сделайте в [п. 2](#2-в-home-assistant) **после** обновления.
